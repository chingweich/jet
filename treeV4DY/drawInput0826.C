...
void drawSDMass (){
  TFile* f1 = TFile::Open("signal-600to4500.root");
  TFile* f2 = TFile::Open("DY.root");
  ...
  TH1F* th23= new TH1F("SDMassCut","SDMassCut",250,0,250);
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(BDT<-990)continue; 
	Float_t SDMass=data.GetFloat("SDMass");
	if(BDT<0)continue;
  ...
  TH1F* th24= new TH1F("SDMassCut","SDMassCut",250,0,250);
  
  
  for (Long64_t jentry=0; jentry<nentries2;jentry++) {
    data2.GetEntry(jentry);
    Float_t BDT=data2.GetFloat("BDT");
    if(BDT<-990)continue; 
	Float_t SDMass=data2.GetFloat("SDMass");
	if(BDT<0)continue;

	th24->Fill(SDMass);
  }
  
  th23->Scale(1/th23->Integral());
  th24->Scale(1/th24->Integral());
  th24->SetLineColor(2);
  th23->Draw();
  th24->Draw("same");
  TLegend* leg ;
...
void drawBase (TFile* f,int drawOpntion=3,string masspoint=""){
  ...
  TH1F* th23= new TH1F("SDMass","SDMass",250,0,250);
  TH1F* th24= new TH1F("SDMassCut","SDMassCut",250,0,250);
...
TH1F* drawE(string s1,string s2,bool show=0){
	...
	int nbins=100;
	TH1F* temp=new TH1F("ROC","ROC",nbins,0,1);
	float CSV=0,sumNum=1e10;
	for(int i=100;i>0 ;i--){
		float tempNum=nentries *i /100;
		//float CSV=1;
		...
		temp->SetBinContent(i,1-(sumNum2/nentries2));
...
TH1F* drawSubCSVBase(string s1,string s2,bool show=0){
	TFile* f1 = TFile::Open(s1.data());
	TFile* f2 = TFile::Open(s2.data());
...
TH1F* drawMixCSVBase(string s1,string s2,bool show=0){
	TFile* f1 = TFile::Open(s1.data());
	TFile* f2 = TFile::Open(s2.data());
	...
	for(int i=100;i>0 ;i--){
		...
		while(sumNum>tempNum){
			...
			Float_t sub2CSV=data.GetFloat("sub2CSV");
			Float_t delta_R=data.GetFloat("delta_R");
			if(delta_R<0.3 &&(sub1CSV > CSV || sub2CSV> CSV))sumNum++;
            if(delta_R>0.3 &&(sub1CSV > CSV && sub2CSV> CSV))sumNum++;
			}
			CSV+=0.005;
			if(CSV>1)break;
		...
		for (Long64_t jentry=0; jentry<nentries2;jentry++) {
			...
			if(delta_R<0.3 &&(sub1CSV > CSV || sub2CSV> CSV))sumNum2++;
            if(delta_R>0.3 &&(sub1CSV > CSV && sub2CSV> CSV))sumNum2++;
...
void drawSubCSV(string s1,string s2,string s3,string s4,string s5,int drawOption){
	
	TH1F* th2[4];
	
		th2[0]=drawSubCSVBase(Form("signal-%s.root",s1.data()),"DY.root");
		th2[1]=drawSubCSVBase(Form("signal-%s.root",s2.data()),"DY.root");
		th2[2]=drawSubCSVBase(Form("signal-%s.root",s3.data()),"DY.root");
		th2[3]=drawSubCSVBase(Form("signal-%s.root",s4.data()),"DY.root");
	...
	leg->AddEntry(th2[3],s4.data());
	leg->Draw("same");
	if(drawOption==1)c1->Print ("pdf/subROC.pdf(");
	else if (drawOption==2)c1->Print ("pdf/subROC.pdf)");
	else c1->Print ("pdf/subROC.pdf");
}

void drawMixCSV(string s1,string s2,string s3,string s4,string s5,int drawOption){
	
	TH1F* th2[4];
	
		th2[0]=drawMixCSVBase(Form("signal-%s.root",s1.data()),"DY.root");
		th2[1]=drawMixCSVBase(Form("signal-%s.root",s2.data()),"DY.root");
		th2[2]=drawMixCSVBase(Form("signal-%s.root",s3.data()),"DY.root");
		th2[3]=drawMixCSVBase(Form("signal-%s.root",s4.data()),"DY.root");
	...
	leg->AddEntry(th2[3],s4.data());
	leg->Draw("same");
	if(drawOption==1)c1->Print ("pdf/mixROC.pdf(");
	else if (drawOption==2)c1->Print ("pdf/mixROC.pdf)");
	else c1->Print ("pdf/mixROC.pdf");
}
...
void drawBDTPt(TFile* f,int drawOption,string masspoint=""){
  ...
  TH2F* th2= new TH2F("BDT",Form("%s BDT",masspoint.data()),20,-0.8,0.8,100,200,3200);
  TH2F* th2cut= new TH2F("BDT",Form("%s BDTcut",masspoint.data()),20,0,1,100,200,3200);
  th2cut->SetXTitle("CSV");
  th2cut->SetYTitle("Pt");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	th2->Fill(BDT,fatPt);
	if(BDT>0) th2cut->Fill(fatCSV,fatPt);
	
  }
  c1->Clear();
  th2->Draw("colz");
  
  if(drawOption==1)c1->Print("pdf/2d.pdf(");
  else if (drawOption==2)c1->Print("pdf/2d.pdf)");
  else c1->Print("pdf/2d.pdf");
  
  
  th2cut->Draw("colz");
  
  if(drawOption==1)c1->Print("pdf/2dcut.pdf(");
  else if (drawOption==2)c1->Print("pdf/2dcut.pdf)");
  
  else c1->Print("pdf/2dcut.pdf");
...
void drawInput0826(){
  ...
  for (int massP=0;massP<13;massP++){
   ...
   if(massP==0) drawBDTPt(f,1,masspoint[massP].data());
   else drawBDTPt(f,3,masspoint[massP].data());
   f->Close();
  }
  ...
  drawEE(TH1,"1600","2000","3000","4500","fatCSV ROC",2);
  ...
  drawSubCSV("600","800","1000","1200","subCSV ROC",1);
  drawSubCSV("1600","2000","3000","4500","subCSV ROC",2);
  
  drawMixCSV("600","800","1000","1200","subCSV ROC",1);
  drawMixCSV("1600","2000","3000","4500","subCSV ROC",2);
